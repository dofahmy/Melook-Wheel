...
async def bot_membership_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Track when a user blocks/unblocks the bot (MY_CHAT_MEMBER updates)."""
    change = getattr(update, "my_chat_member", None)
    if change is None:
        return

    user = getattr(change, "from_user", None) or getattr(update, "effective_user", None)
    if user is None:
        return
    ...
    # Telegram reports a blocked bot as KICKED/BANNED; LEFT is also unavailable.
    if status in ("kicked", "banned", "left"):
        database.set_telegram_delivery_status(user_id, "blocked", f"my_chat_member:{status}")
        return

    # Any normal membership state means the user can interact with the bot again.
    if status in ("member", "administrator", "creator", "owner", "restricted"):
        try:
            database.upsert_user(user_id, getattr(user, "username", None), getattr(user, "first_name", None))
        except TypeError:
            database.upsert_user(user_id, getattr(user, "username", None))
