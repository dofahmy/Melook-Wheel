...
def init_db():
    with get_conn() as conn:
        ...
        conn.execute(
            "UPDATE users SET golden_target=? WHERE golden_target> ?",
            (GOLDEN_TARGET_COUNT, GOLDEN_TARGET_COUNT),
        )
...
def create_lucky_spin(user_id: int, prize: float | None = None) -> tuple[int, float, int]:
    ...
    with get_conn() as conn:
        ...
        # A pending spin from a finished round remains claimable after progress
        # resets. Creating a *new* spin requires five correct answers.
        progress = conn.execute(
            "SELECT golden_opened_count, golden_answered_count, golden_target, anchor_round_active FROM users WHERE user_id=?",
            (user_id,),
        ).fetchone()
        if not progress or int(progress["golden_opened_count"] or 0) < int(progress["golden_target"] or 5):
            raise ValueError("لا يمكن إنشاء جائزة قبل خمس إجابات صحيحة")
...
def answer_golden_question(user_id: int, question_id: int, chosen_index: int):
    ...
    with get_conn() as conn:
        ...
        # Normal and bonus rounds have no fixed minimum. Their rewards stay
        # proportional to operational EPC and EGYPT_CUSTOMER_REWARD_RATE.
        # Wrong answers contribute zero; only the one-time welcome round above
        # is replaced with the fixed 2.00 EGP reward.
...
def list_customer_reports(period: str = "all", search: str = "", limit: int = 200, offset: int = 0,
    ...
    """
    if (period or "").lower() == "online":
        joined_where = "u.last_activity_at IS NOT NULL AND datetime(u.last_activity_at) >= datetime('now','-5 minutes')"
        joined_params = []
    else:
...
def count_customer_reports(period: str = "all", search: str = "", date_from: str | None = None, date_to: str | None = None) -> int:
    """عدد صفوف قائمة العملاء بعد تطبيق نفس فلتر الفترة والبحث."""
    if (period or "").lower() == "online":
        joined_where = "u.last_activity_at IS NOT NULL AND datetime(u.last_activity_at) >= datetime('now','-5 minutes')"
        joined_params = []
    else:
    ...
    with get_conn() as conn:
        row = conn.execute(f"""
            ...
            WHERE u.program='egypt'
              AND {joined_where}
              AND (?='' OR CAST(u.user_id AS TEXT) LIKE ? OR COALESCE(u.username,'') LIKE ? OR COALESCE(wa.phone_e164,'') LIKE ?)
        """, (*joined_params, search, like, like, like)).fetchone()
        return int(row["c"] or 0)
...
def get_customer_list_stats(period: str = "all", date_from: str | None = None, date_to: str | None = None) -> dict:
    ...
    if (period or "").lower() == "online":
        joined_where = "last_activity_at IS NOT NULL AND datetime(last_activity_at) >= datetime('now','-5 minutes')"
        joined_params = []
    else:
